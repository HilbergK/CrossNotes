  ...
  startActivityForResult(std::move(clipSelection), [this, bookTitle = std::move(bookTitle), author = std::move(author),
    ...
    if (!result.isCancelled) {
      ...
      if (!clip.text.empty()) {
        ...
        // CrossInk Notes: capture the new clipping's identity for the tag picker
        // before this scope unwinds.
        if (saved) {
          if (const Clipping* added = CLIPPINGS.clippingAt(clippingIndex)) {
            tagSpine = added->spineIndex;
