...
// CrossInk Notes: 1024 (upstream 512) ≈ 150-180 words per highlight. Since
// v1.5.0 clipping text lives outside the record (textOffset/textLength) and is
// read on demand, so a larger *read* cap no longer costs resident RAM.
//
// Upstream rejects the WHOLE file if any record's textLength exceeds 512, so
// a long highlight written here made stock CrossInk read that book as empty;
// a new highlight there then rewrote the file with only that one. New captures
// are truncated to CLIPPING_TEXT_PORTABLE_MAX at the addClipping call site
// (owned). This read cap stays 1024 so books we already wrote still load.
// Existing long records are not rewritten.
inline constexpr size_t CLIPPING_TEXT_MAX = 1024;          // read: accept what we already wrote
inline constexpr size_t CLIPPING_TEXT_PORTABLE_MAX = 512;  // write: upstream's cap
