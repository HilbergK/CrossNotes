...
### Added
- Filter notes by tag.
- Sort a book's notes by date added or by location in book (hold the menu button in a notes page).
- **Notes and Bookmarks** shows a highlight, note and bookmark count per book.
- Copy individual highlights, notes, or both to the clipboard from the website.

### Changed
- After writing a note on a new highlight you can now also tag it, instead of choosing one or the other.
...
### Fixed
- Opening a highlight from the notes list no longer opens the book menu on top of it. Credit and thanks to memeeme22 on Reddit for finding and reporting it.
- Menus in Notes and Bookmarks were at the wrong level.
- Marginal cases where notes could be deleted.
